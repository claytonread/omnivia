...
class RelationshipRepository:
    ...
    def get_neighbors(self, entity_id: str) -> list[tuple[Entity, Relationship]]:
        ...
        cursor = self.db.execute(
            ...
            SELECT
                ...
                e.entity_type AS neighbor_entity_type,
                e.source_id AS neighbor_source_id,
                e.approval_status AS neighbor_approval_status,
                e.created_at AS neighbor_created_at,
                e.updated_at AS neighbor_updated_at
        ...
        for row in cursor.fetchall():
            ...
            entity = self._row_to_neighbor_entity(row)
    ...
    def _row_to_neighbor_entity(self, row: Any) -> Entity:
        """Convert aliased neighbor columns from a relationship join."""
        return Entity(
            id=row["neighbor_id"],
            name=row["neighbor_name"],
            entity_type=EntityType(row["neighbor_entity_type"]),
            source_id=row["neighbor_source_id"],
            approval_status=ApprovalStatus(row["neighbor_approval_status"]),
