...
class MemoryService:
    ...
    def search(
        ...
        return self.repository.search(query, limit, workspace_id=workspace_id)

    def list_by_workspace(self, workspace_id: str) -> List[Memory]:
        """List all memories associated with a workspace."""
        if not self.repository:
            raise MemoryServiceError("Repository not configured")
